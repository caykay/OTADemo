...
static esp_err_t onWrite(const HTTPUploadStatus status, uint8_t *buffer,
                         const size_t bufferSize) {
  const esp_partition_t *bootPartition = esp_ota_get_boot_partition();
  const esp_partition_t *runningPartition = esp_ota_get_running_partition();
  if (bootPartition != runningPartition) {
    Serial.printf("Configured OTA boot partition at offset 0x%08x, but running "
                  "from offset 0x%08x",
                  bootPartition->address, runningPartition->address);
    Serial.println("(This can happen if either the OTA boot data or preferred "
  ...
  case UPLOAD_FILE_START: {
    ...
    if (!updatePartition) {
      Serial.println("[OTA] esp_ota_get_next_update_partition: Could not "
                     "configure firmware partition.");
      return ESP_ERR_NOT_FOUND;
    }
    // size_t chunkSize = sizeof(uint8_t) * HTTP_UPLOAD_BUFLEN;
    esp_err_t result =
        esp_ota_begin(updatePartition, OTA_SIZE_UNKNOWN, &handle);
    if (result != ESP_OK) {
      Serial.println(
          "[OTA] esp_ota_begin: failed to begin firmware update service.");
      esp_ota_abort(handle);
    }
    return result;
  }
  case UPLOAD_FILE_WRITE: {
    esp_err_t result = esp_ota_write(handle, (void *)buffer, bufferSize);
    if (result != ESP_OK) {
      Serial.println("[OTA] esp_ota_write: could not write firmware data to "
                     "firmware partition.");
      esp_ota_abort(handle);
    }
    return result;
  ...
  case UPLOAD_FILE_END: {
    ...
    if (result != ESP_OK) {
      Serial.println("[OTA] esp_ota_end: could not finalize firmware update, "
                     "likely that firmware image is invalid");
      esp_ota_abort(handle);
    } else {
      result = esp_ota_set_boot_partition(updatePartition);
      if (result != ESP_OK) {
        Serial.println("[OTA] esp_ota_set_boot_partition: could not set the "
